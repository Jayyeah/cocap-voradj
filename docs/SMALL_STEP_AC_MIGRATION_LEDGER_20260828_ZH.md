...
| PPO-CF seed1/2 | COMPLETE | 两seed 400k，deterministic capture 均 `0/20`，Q/critic/PPO 数值健康 | 2-seed final，已封板 |
| PPO-CF seed3 | STOPPED_BY_USER | 用户要求取消，未启动；supervisor/tmux 已关闭 | 不再启动 |

GPU0 当前显存 `15/48525 MiB`、无 compute app；GPU1 显存 `17029/49140 MiB`，唯一 compute app 为范围外 OmniVLA PID `487833`。PPO-CF supervisor 已按用户要求关闭，不会影响该外部服务。当前磁盘约 `40.3 GiB` 可用、RAM available 约 `100.6 GiB`。PPO-CF 不再有剩余训练线或 ETA。
...
| 5 | **2-seed final / healthy negative** | PPO-CF seed1/2 均完成 `400k`，deterministic capture 均 `0/20`；但 Q chosen/baseline、Q span、critic loss、explained variance、A_CF、梯度与 KL 均 finite/健康（seed1 EV `.976`、Q span `1.34`；seed2 EV `.826`、Q span `1.15`）。用户已明确停止 seed3，故在“2-seed final”范围内封板为 healthy negative：简单 V→counterfactual Q 未带来 capture 改善，不再机械堆更复杂 critic。该结论不冒充原三 seed gate，适用范围明确为已完成的两 seed。 |
