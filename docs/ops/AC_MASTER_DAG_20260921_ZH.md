...
- MASTER branch：`ops/ac-master-dag-20260921`，本地 HEAD `c2c4811`，origin 基线 `4538aca`，当前 local-ahead 记录待用显式 proxy 推送；基于 `ops/training-performance-sync-20260921`。后续 push 每次必须显式使用 mihomo `127.0.0.1:17892` 的 `http_proxy/https_proxy/all_proxy` 与 `git -c http.proxy/-c https.proxy`，禁止 stale `17891`。
