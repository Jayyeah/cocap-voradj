...
- [x] B0：冻结 Final IQN canonical baseline；
- [x] B1：实现/准备删除 `is_pursuing` 的 student；
- [x] 优先做 distillation / BC，不从 scratch；
- [x] matched rollout 验证 `is_pursuing` 是否真是必要 shortcut。
...
## B0/B1 状态回写（2026-09-17）

### B0 — COMPLETE / FROZEN

- Final IQN teacher：`artifacts/2026-08-04_crms_vctls_ce_final/checkpoints/stage1_4v1_step_2000000.pt`。
- SHA-256：`2f39ea046acb56321585e7bbe1a8f0869a4dd85777ff2ae65ca29a7bd5680c89`。
- 复用可信 C0 20-pair 与 C3 100-pair matched rollout；reference contract、AW9、observation/reward/sensing/backbone 与 `is_pursuing` 完整路径见 `docs/B0_FINAL_IQN_REFERENCE_20260917_ZH.md` 及对应 JSON artifact。

### B1 — COMPLETE / GATE FAIL

- 已实现 backward-compatible loader、no-is-pursuing config、role-free physical friend ordering、student weight transfer、regression tests 与 parity artifacts。
- teacher frozen distillation/BC 后，full C1 146,612 rows action agreement `0.3738`；pre/post/pure-coverage 分别 `0.3982/0.3652/0.3689`，support `0.2773`。
- matched rollout：3 mixed + 3 coverage；mixed capture `3/3` 但 recovery `0/3`，coverage safe completion `0/3`；student trajectory teacher agreement `0.1722`。
- calibrated physical near-pair audit（RMSE≤0.10）：118 cross-role pairs，87 action mismatches（`73.7%`）；主要指向 target recent memory / local target evidence。严格 RMSE≤0.05 无 pair，故该解释已区分为 calibrated evidence，不冒充 exact aliasing。
- Gate：**`B1_FAIL_HIDDEN_STATE_REQUIRED`**。`is_pursuing` 不恢复到 policy；本轮不进入 B2，不实现 local Voronoi、`z_i`、reward 改造或 scratch long training。`B2` 仍需人工审阅 hidden-state 来源后单独授权。
- 机器可读总结果：`artifacts/2026-09-17_iqn_cleanup_b1/B1_FINAL_AUDIT.json`；详细审计：`docs/B1_IQN_CLEANUP_AUDIT_20260917_ZH.md`。
