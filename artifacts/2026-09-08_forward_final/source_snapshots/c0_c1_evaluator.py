...
@torch.no_grad()
def run_episode(model,scene,seed,device,*,max_steps=None,on_transition=None,on_progress=None):
    ...
    for step in range(1,horizon+1):
        ...
        for i,a in zip(active,greedy):commands[i]=int(a)
    ...
    return {'scene':scene,'seed':int(seed),'initial_state_fingerprint':fingerprint,'contract':CONTRACT,
        ...
        'mission_events':tracker.finish(step),'smoke_horizon_override':max_steps,
