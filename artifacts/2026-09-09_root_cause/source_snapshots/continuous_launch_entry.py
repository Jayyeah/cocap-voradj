...
def main():
    p=argparse.ArgumentParser();p.add_argument('--out',type=Path,required=True);p.add_argument('--device',default='cuda:0');p.add_argument('--probe-only',action='store_true');a=p.parse_args();out=a.out
    ...
    for mode in ('mean','sample'):
        for scene in ('mixed','coverage'):
            ...
            if mode=='mean':
                ...
                checks[mode+'/'+scene+'/p90']=s['full_mission_seconds_completed']['p90'] is not None and s['full_mission_seconds_completed']['p90']<=1.25*np.percentile(rt,90)
