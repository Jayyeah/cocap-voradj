...
if __name__=='__main__':
    try:main()
        ...
        # A dead worker must not leave an apparently running progress/ETA behind.
        if '--out' in sys.argv:
            failed_out=Path(sys.argv[sys.argv.index('--out')+1])
            launch_path=failed_out/'launch.json'
            owns_run=launch_path.exists() and json.loads(launch_path.read_text()).get('pid')==os.getpid()
            if not owns_run:raise  # Preserve a pre-existing run rejected by the fresh-output check.
            progress_path=failed_out/'progress.json'
            previous=json.loads(progress_path.read_text()) if progress_path.exists() else {}
            atomic_json(failed_out/'failure_audit.json',{'status':'failed','error':type(exc).__name__+': '+str(exc),'traceback':traceback.format_exc(),'last_progress':previous,'pid':os.getpid()})
            atomic_json(progress_path,{'status':'failed','last_progress':previous,'eta_seconds':None,'failure_audit':'failure_audit.json'})
        raise
