...
def smoke(c, out, seed, device):
    ...
    resumed_metrics = trainer.update(next_batch, categorical=True)
    assert all(np.isfinite(v) for v in resumed_metrics.values())
    restore_rng(update_rng, clone.device)
    clone_metrics = clone.update(next_batch, categorical=True)
    assert_tree_equal(resumed_metrics, clone_metrics)
    assert_tree_equal(trainer.state_dict(), clone.state_dict())
    eval_records = []
    ...
    report = dict(status='SMOKE_PASS', formal_training='HOLD', seed=seed, device=device,
                  ...
                  runtime=runtime, metrics=metrics, resumed_update_metrics=resumed_metrics,
                  resume_transition_rng_optimizer_bit_exact=True,
...
def main():
    ...
    write_json(args.output / 'progress.json', dict(status=result['status'], formal_training='HOLD'))
